...
+ (CoreTextData *)parseTemplateFile:(NSString *)path
    ...
    NSAttributedString *content = [self loadContentFromFile:path config:config imageArray:imageArray linkArray:linkArray];
...
+ (NSAttributedString *)loadContentFromFile:(NSString *)path
    ...
    // 此处直接用一个数组(数组嵌字典)表示。(具体解析方式应根据网络接口传回的数据来设置)
